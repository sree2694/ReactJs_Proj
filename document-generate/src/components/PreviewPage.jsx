...
import headerImage from './slogo.jpeg';
...
const PreviewPage = () => {
  ...
  const handleGeneratePDF = () => {
    ...
    const headerImageWidth = 50; // Width of the header image
  ...
  `;
    ...
    while (remainingLines.length > 0) {
      // Add header image
      const headerImageX = pageWidth - marginRight - headerImageWidth; // X position of the header image
      doc.addImage(headerImage, 'JPEG', headerImageX, cursorY, headerImageWidth, 0); // Height of the image is calculated automatically based on aspect ratio
  
      // Add content
      const contentY = cursorY;
      const availableLines = Math.floor((pageHeight - contentY - marginBottom) / 6);
      const currentLines = remainingLines.slice(0, availableLines);
      doc.setFontSize(12);
      doc.text(currentLines, marginLeft, contentY);
